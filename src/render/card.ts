// The card template: an IndexedItem becomes exactly ROWS lines of at most COLS
// columns, which renderGrid turns into a fixed 1024x1024 PNG (64 cols x 32
// rows of 8x16 cells at scale 2). Everything on the card is derived from
// on-chain data already sitting in the index, so the image is deterministic
// and immutable-cacheable. Mutable fields (supply, holders, stars) are
...
export function cardLines(item: IndexedItem): string[] {
  const attrs: Trait[] = item.attributes ?? [];
  const category = attrs.find((t) => t.trait_type === "category")?.value;
  const hashtags = attrs.filter((t) => t.trait_type === "skill").map((t) => t.value);
  ...
  const full = wordmark(item.name);
  const firstToken = item.name.split("-")[0] ?? item.name;
  const short = wordmark(firstToken);
  if (full && colsOf(full[0]) <= INNER) {
    head.push(INDENT + full[0], INDENT + full[1]);
  } else if (short && colsOf(short[0]) <= INNER && firstToken.length < item.name.length) {
    head.push(INDENT + short[0], INDENT + short[1]);
    const nameLines = wrapSlug(item.name, INNER - 2).slice(0, 3);
    head.push(...nameLines.map((l, i) => INDENT + (i === 0 ? "> " : "  ") + l));
  } else {
    head.push(...wrapSlug(item.name, INNER - 2).slice(0, 3).map((l, i) => INDENT + (i === 0 ? "> " : "  ") + l));
  }

  const rule = INDENT + "─".repeat(INNER);
  head.push("", rule, "");
  head.push(INDENT + `[ ${(category ?? "").toUpperCase()}${category ? " / " : ""}${item.type.toUpperCase()} ]`, "");

  const foot: string[] = [
    rule,
    "",
    INDENT + `creator    ${truncMiddle(item.creator)}`,
    INDENT + `price      ${lamportsToSol(item.price)}`,
    "",
    footerLine(),
    "",
    "╚" + "═".repeat(COLS - 2) + "╝",
  ...
  const tagLine = hashtags.length ? fitTags(hashtags) : null;
  let budget = ROWS - head.length - foot.length - (tagLine ? 2 : 0);
  const desc = wrap(item.description ?? "", INNER);
  const body: string[] = [];
...
export function renderCard(item: IndexedItem): Buffer {
  return renderGrid(cardLines(item), { cols: COLS, rows: ROWS, scale: 2 });
}
